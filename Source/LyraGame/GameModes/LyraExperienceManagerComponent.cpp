...
//这是一个测试Experience随机延迟的命令行参数,它可以通过命令行输入读取随机延迟时间的最小值和最大值,并通过这个GetExperienceLoadDelayDuration()去获取到一个随机测试值.
...
void ULyraExperienceManagerComponent::EndPlay(const EEndPlayReason::Type EndPlayReason)
{
	...
	if (LoadState == ELyraExperienceLoadState::Loaded)
	{
		...
		// 确保即便有人注册为暂停者但随即立即执行操作，我们也不会过早完成转换过程。
...
void ULyraExperienceManagerComponent::OnActionDeactivationCompleted()
{
	...
	++NumObservedPausers;
	// 所有的Action都已观测到取消,这里应该时异步的,但是代码给的日志,提示目前还不支持异步的操作
